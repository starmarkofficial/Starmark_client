...
const [_id, set_id] = useState("")
	...
	const userCreate = async(e)=>{		
			...
			//   profileData.append("Married", perInfo.married);
			  ...
			  const {data} = await axios.put(`${baseUrl}/update-profile/${_id}`,profileData)
			  if (data?.success) {
                alert("updated")
			  } else {
				alert("h")
  ...
  return (
    ...
    <fieldset className="grid grid-cols-4 gap-6 p-6  rounded-xl py-10 px-6 outline-none shadow-lg shadow-gray-400 bg-gray-200">
			...
			<div className="space-y-2 col-span-full lg:col-span-1 p-5 font-bold">
				<p className="font-bold text-3xl text-blue-700">Update Profile </p>
		...
		<fieldset className="grid grid-cols-4 gap-6 p-6  rounded-xl py-10 px-6 outline-none shadow-lg shadow-gray-400 bg-gray-200">
			...
			<div className="grid grid-cols-6 gap-4 col-span-full lg:col-span-3">
				...
				<div className="col-span-full sm:col-span-3">
					...
					<input id="email" type="email" name="email"  value={perInfo.email} placeholder="Email" className="w-full rounded-lg focus-none outline-none py-3 px-2 outline-none bg-gray-100  border-gray-300 text-gray-400" />
				</div>
				<div className="col-span-full sm:col-span-3">
